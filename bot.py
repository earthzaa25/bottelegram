...
def build_report():
    ...
    urg = '\n'.join([f'  • {r["unit"]} — {r["name"][:35]}... (เหลือ {r["days"]} วัน)' for r in sorted(urgent, key=lambda x: int(x['days']))])
    urgent_block = f"\n🔴 งานใกล้ครบกำหนด:\n{urg}\n" if urg else ""
    report = (
...
def build_dashboard_image():
    total, budget, units, urgent = get_summary()
    fig = plt.figure(figsize=(12, 8), facecolor='#0a1628')
    fig.suptitle('Dashboard จัดซื้อจัดจ้าง กบ.ทหาร', fontsize=16, color='white', fontweight='bold', y=0.98)
    ...
    kpi_data = [
        ...
        ('หน่วยที่รับผิดชอบ', f'{len(units)}', '#00c896'),
    ]
    for i, (label, val, color) in enumerate(kpi_data):
        ax = fig.add_axes([0.04 + i*0.245, 0.78, 0.22, 0.14])
        ax.set_facecolor('#0f2044')
        ax.set_xlim(0,1); ax.set_ylim(0,1)
        ax.axhline(y=1, color=color, linewidth=3)
        ax.text(0.5, 0.65, val, ha='center', va='center', color=color, fontsize=18, fontweight='bold')
        ax.text(0.5, 0.2, label, ha='center', va='center', color='#8a9bb8', fontsize=9)
        ax.axis('off')

    # Donut chart - หน่วย
    ax1 = fig.add_axes([0.03, 0.32, 0.3, 0.42])
    ...
    wedges, _ = ax1.pie(unit_vals, colors=colors[:len(unit_vals)], startangle=90,
                        wedgeprops=dict(width=0.5))
    ax1.set_title('สัดส่วนตามหน่วย', color='#8a9bb8', fontsize=10, pad=8)
    legend = [mpatches.Patch(color=colors[i], label=f'{unit_names[i]} ({unit_vals[i]})') for i in range(len(unit_names))]
    ax1.legend(handles=legend, loc='lower center', bbox_to_anchor=(0.5,-0.25),
               ncol=2, fontsize=7, labelcolor='white', facecolor='#0f2044', edgecolor='none')

    # Bar chart - วงเงินตามหน่วย
    ax2 = fig.add_axes([0.38, 0.32, 0.58, 0.42])
    ax2.set_facecolor('#0f2044')
    unit_budget = {}
    for r in PROCUREMENT_DATA:
        unit_budget[r['unit']] = unit_budget.get(r['unit'], 0) + r['budget']
    sorted_units = sorted(unit_budget.items(), key=lambda x: x[1], reverse=True)
    names = [x[0] for x in sorted_units]
    vals  = [x[1]/1e6 for x in sorted_units]
    bars  = ax2.barh(names, vals, color='#00d4ff', alpha=0.8)
    ...
    ax2.set_title('วงเงินตามหน่วย (ล้านบาท)', color='#8a9bb8', fontsize=10, pad=8)
    ax2.set_xlim(0, max(vals)*1.2)

    # Urgent table
    ax3 = fig.add_axes([0.03, 0.03, 0.94, 0.24])
    ax3.set_facecolor('#0f2044')
    ax3.set_xlim(0,1); ax3.set_ylim(0,1); ax3.axis('off')
    ax3.text(0.01, 0.92, '⚠️  งานต้องติดตามเร่งด่วน', color='#ff4d6d', fontsize=10, fontweight='bold', va='top')
    for i, item in enumerate(URGENT_ITEMS[:4]):
        color = '#ff4d6d' if i < 2 else '#f5a623'
        ax3.text(0.01, 0.68 - i*0.18, f'  • {item}', color=color, fontsize=9, va='top')

    ax3.text(0.98, 0.05, f'ข้อมูล ณ {datetime.now().strftime("%d/%m/%Y")}  |  กบ.ทหาร',
             color='#3a4a6a', fontsize=8, ha='right', va='bottom')

    buf = io.BytesIO()
    plt.savefig(buf, format='png', dpi=130, bbox_inches='tight', facecolor='#0a1628')
    buf.seek(0)
    plt.close()
    return buf
...
async def cmd_dashboard(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📊 กำลังสร้าง Dashboard...")
    buf = build_dashboard_image()
    await update.message.reply_photo(photo=buf, caption=f"📊 Dashboard จัดซื้อจัดจ้าง กบ.ทหาร\nข้อมูล ณ {datetime.now().strftime('%d/%m/%Y %H:%M น.')}")
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if any(w in text for w in ['dashboard','แดชบอร์ด','กราฟ','ภาพ','รูป']):
        ...
        # ค้นหาตามชื่อหน่วย
        found = [r for r in PROCUREMENT_DATA if text in r['unit'].lower() or text in r['name'].lower()]
        if found:
            ...
            await update.message.reply_text(
                "ไม่เข้าใจคำถามครับ ลองพิมพ์คำสั่งเหล่านี้:\n"
...
def main():
    ...
    # ส่งรายงานอัตโนมัติทุกวัน 08:00 น.
